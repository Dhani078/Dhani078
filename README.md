<div align="center">

![Hero Ultimate](https://raw.githubusercontent.com/Dhani078/Dhani078/main/assets/hero-ultimate.svg)

[![Portfolio](https://img.shields.io/badge/🌐_Portfolio-portofoliodan--lovat.vercel.app-00d4ff?style=for-the-badge&logo=vercel&logoColor=white)](https://portofoliodan-lovat.vercel.app)
[![Email](https://img.shields.io/badge/📧_Email-dhanisepeda@gmail.com-ff0080?style=for-the-badge&logo=gmail&logoColor=white)](mailto:dhanisepeda@gmail.com)
[![WhatsApp](https://img.shields.io/badge/💬_WhatsApp-+62_821--4856--4979-00ff88?style=for-the-badge&logo=whatsapp&logoColor=white)](https://wa.me/6282148564979)

</div>

---

## 👨‍💻 About

```typescript
const developer = {
  name: "Muhammad Rizki Ramadhani",
  role: "Full-stack Software Engineer",
  location: "Banjarmasin, Indonesia 🇮🇩",
  education: "Computer Science • Semester 7 • UNISKA",
  
  building: [
    "Scalable edge applications (Cloudflare Workers)",
    "3D interactive experiences (Three.js + React Three Fiber)",
    "Serverless architectures (TiDB Cloud Serverless)",
    "Offline-first Progressive Web Apps"
  ],
  
  deployed: "5 production applications",
  status: "🟢 Available for Software Engineering Internship"
};
```

---

## 🛠️ Tech Arsenal

<div align="center">

### Frontend Engineering
**Next.js 16** (App Router, RSC) • **React 19** (Server Actions) • **TypeScript** (Strict)  
**Three.js / R3F** • **Rapier Physics** • **Tailwind CSS v4** • **Framer Motion**

### Backend & Cloud
**Cloudflare Workers** • **Node.js** • **Edge Runtime**  
**TiDB Cloud Serverless** • **Supabase** (PostgreSQL + RLS) • **Drizzle ORM**

### DevOps & Tools
**Git & GitHub** • **Docker** • **Linux** • **Vercel** • **CI/CD Pipelines** • **PWA**

</div>

---

## 🚀 Production Portfolio

<div align="center">

### EquipRent Management System

[![EquipRent Cyberpunk Card](https://raw.githubusercontent.com/Dhani078/Dhani078/main/assets/card-cyber-equiprent.svg)](https://equiprent-pt-surya-bangun-sarana.dhanisepeda.workers.dev)

**[→ LIVE DEMO](https://equiprent-pt-surya-bangun-sarana.dhanisepeda.workers.dev)** • **[→ GitHub](https://github.com/Dhani078/equiprent-pt-surya-bangun-sarana)**

Heavy equipment rental management system for PT. Surya Bangun Sarana with GPS tracking, role-based authentication (Admin/Staff/User), real-time inventory management, and transaction processing. Deployed globally on Cloudflare edge network.

**Stack**: React 18 • TypeScript • Cloudflare Workers • TiDB

---

### Embun Laundry

[![Embun Card](https://raw.githubusercontent.com/Dhani078/Dhani078/main/assets/card-embun-3d.svg)](https://embun-laundry.dhanisepeda.workers.dev)

**[→ LIVE DEMO](https://embun-laundry.dhanisepeda.workers.dev)** • **[→ GitHub](https://github.com/Dhani078/dhani-laundry)**

Modern laundry management system with real-time order tracking, customer database, payment processing, and analytics dashboard. Serverless architecture for zero-maintenance scaling.

**Stack**: Cloudflare Workers • TiDB • JavaScript

---

### KasDesk PWA

[![KasDesk Card](https://raw.githubusercontent.com/Dhani078/Dhani078/main/assets/card-kasdesk-3d.svg)](https://kas-desk.vercel.app)

**[→ LIVE DEMO](https://kas-desk.vercel.app)** • **[→ GitHub](https://github.com/Dhani078/KasDesk)**

Personal finance & wealth tracker with offline-first PWA architecture, multi-wallet transaction tracking, OCR receipt scanning (Gemini Vision API), biometric authentication, and real-time sync with TiDB Cloud.

**Stack**: Next.js 16 • React 19 • TiDB • Drizzle ORM • PWA

---

### DAN.DEV Portfolio

**[→ LIVE DEMO](https://portofoliodan-lovat.vercel.app)** • **[→ GitHub](https://github.com/Dhani078/Portofolio)**

3D interactive portfolio with physics-based ID card lanyard using React Three Fiber + Rapier Physics. UV-mapped front/back card design, smooth animations, and responsive 3D experience.

**Stack**: Next.js 16 • React 19 • Three.js • Rapier Physics • Supabase

---

### AutoClip

**[→ GitHub](https://github.com/Dhani078/AutoClip)**

Video transcription and auto-captioning pipeline with speech-to-text, frame extraction, and automated subtitle generation.

**Stack**: FastAPI • OpenCV • Faster-Whisper

---

**[→ All Repositories](https://github.com/Dhani078?tab=repositories)**

</div>

---

## 📊 GitHub Activity

<div align="center">

**42 Total Contributions** • **5 Production Apps** • **4 Tech Stacks Mastered**

[![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Dhani078&show_icons=true&theme=tokyonight&hide_border=true&title_color=00d4ff&icon_color=00d4ff&text_color=c9d1d9&bg_color=0d1117)](https://github.com/Dhani078)

**Primary Languages:** TypeScript • JavaScript • PHP • Python

</div>

---

## 💼 Seeking Internship

```yaml
role:
  - Software Engineer
  - Full-stack Developer
  - System Engineer
  - Cloud/Edge Engineer

interests:
  - Web development (Next.js, React, TypeScript)
  - Serverless & edge computing (Cloudflare Workers)
  - 3D web experiences (Three.js, React Three Fiber)
  - Database architecture (TiDB, PostgreSQL)
  - Cloud infrastructure & DevOps

location: Banjarmasin, Indonesia
work_type: [on-site, hybrid, remote]
availability: immediate
focus: real coding projects with portfolio output
```

**Target Companies**:
- **OJT Informatika** — Linux, Docker, Open Journal Systems
- **BanjarKode** — Software development
- **iTech Space** — Tech innovation
- **Cipta Dewantara** — Enterprise systems
- **Government** — Bappeda Litbang, Disbudporapar, DPMPTSP, Disdukcapil

---

## 📬 Contact

<div align="center">

**Email**: [dhanisepeda@gmail.com](mailto:dhanisepeda@gmail.com)  
**WhatsApp**: [+62 821-4856-4979](https://wa.me/6282148564979)  
**Portfolio**: [portofoliodan-lovat.vercel.app](https://portofoliodan-lovat.vercel.app)  
**GitHub**: [@Dhani078](https://github.com/Dhani078)

---

![Profile Views](https://komarev.com/ghpvc/?username=Dhani078&label=views&color=00d4ff&style=flat-square)

**Muhammad Rizki Ramadhani** • TI Semester 7 UNISKA Banjarmasin  
🟢 Available for internship opportunities

</div>
